class ChoicesController < InheritedResources::Base
  VALID_ORDER_FIELDS = %w(losses wins score created_at prompts_count).freeze
  ...
  def build_order_from_params
    field = VALID_ORDER_FIELDS.include?(params[:orderby]) ? params[:orderby] : 'score'
    direction = VALID_ORDER_DIRECTIONS.include?(params[:orderdirection]) ? params[:orderdirection] : 'desc'
    "#{field} #{direction}"
  end
